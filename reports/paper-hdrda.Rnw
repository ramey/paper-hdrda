...
\subsection{Relationship with Other Classifiers}

The addition of the parameter $\alpha_k$ in \eqref{eq:hdrda-cov} is one of
convenience and allows the flexibility of various covariance-matrix estimators
proposed in the literature. In practice, we generally are not interested in
estimating $\alpha_k$ as it would incorporate $K$ additional tuning parameters
to estimate via cross-validation, which is counterproductive to our goal of
improving the \emph{RDA} classifier's efficiency. By setting $\alpha_k$ to a
predefined constant, the \emph{HDRDA} covariance-matrix estimator includes or
resembles a large family of estimators, some of which we mention below.


\subsubsection{\cite{Friedman:1989tm}}
...
\cite[Chapter~18.3.5]{Hastie:2008dt}. Recall that the pooled sample covariance
matrix is $\widehat{\bm \Sigma} = \bm X^{T} \bm X$\footnote{TODO: $1/N$ is
  missing. Add this. It will be absorbed into the eigenvalues.}  Then, because
$\bm X = \bm M \bm D^{1/2} \bm U^{T}$ by the SVD, we have the eigendecomposition
$\widehat{\bm \Sigma} = \bm U \bm D \bm U^{T}$, where $\bm M \in \mathbb{R}_{N
...
\mathbb{R}_{p \times N}$ is orthogonal. Now, $\bm X \bm X^{T} = \bm M \bm D \bm
M^{T}$ is the eigenvalue decomposition of $\bm X \bm X^{T}$, where $\bm D$ is
the matrix of eigenvalues of $\bm X \bm X^{T}$ and the columns of $\bm M$ are
the corresponding eigenvectors. Hence, we obtain $\bm M$ and $\bm D$ from an
...
Q_k$ requires $O(TODO)$ operations. Because $\bm Q_k \in \mathbb{R}_{n_k \times
  n_k}$, both its determinant and inverse require $O(n_k^3)$
operations. Consequently, $\bm W_k^{-1}$ requires $O(TODO)$ operations. Also,
the inverse and determinant of the diagonal matrix $\bm \Gamma_k \in
\mathbb{R}_{q \times q}$ requires $O(q)$ operations. Therefore, the $| \bm W_k
|$ requires $O(q n_k^3)$ operations.
