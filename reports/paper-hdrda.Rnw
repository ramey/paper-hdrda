...
grid search. We formulate a new RDA-based classifier for the small-sample,
...
offers interpretability that was previously lacking with the RDA classifier. We
...
present, the Guo classifier's average error rate was not competitive with
the \emph{HDRDA} classifiers or the random-forest classifier.
...
random-forest method increased in average error rate as $p$
increased. Contrarily, the performance of the \emph{HDRDA} and Guo classifiers
was hardly affected by $p$.  Also, as discussed above, the \emph{HDRDA}
classifiers were superior to all other classifiers for large values of $p$ with
only the random-forest classifier outperforming them in smaller
feature-dimension cases.
...
comparable across all six data sets. Furthermore, the average error rates for the
