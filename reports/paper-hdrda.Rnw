...
\usepackage{graphicx, amsmath, amssymb, bm, mathtools, amsthm, natbib, setspace}
\usepackage{algorithm2e, authblk, url}
...
\author[a]{John A. Ramey}
\author[b]{Caleb K. Stein}
\author[c]{Phil D. Young}
\author[d]{Dean M. Young}
\affil[a]{uStudio, Inc.}
\affil[b]{Myeloma Institute, University of Arkansas for Medical Sciences}
\affil[c]{Department of Management and Information Systems, Baylor University}
\affil[d]{Department of Statistical Science, Baylor University}
...
estimated via a computationally burdensome cross-validation procedure that uses
a grid search. We formulate a new RDA-based classifier for the small-sample,
...
Additionally, \cite{Ching:2012fu}, \cite{Zhang:2010va}, \cite{Ye:2009gd},
...
assumed that the features within each class are independent so that the
population covariance matrices are diagonal, resulting in positive-definite
sample covariance matrices. For excellent overviews of the regularization
problem in discriminant analysis and other techniques, see \cite{Murphy:2012uq}
...
reparameterizing the \emph{RDA} classifier similar to that of
\cite*{Hastie:2008dt} and \cite{Halbe:2007ea}, we show that the pooling
parameter in the \emph{HDRDA} classifier determines the contribution of each
training observation to the estimation of each class covariance matrix, enabling
interpretability that has been previously lacking with the \emph{RDA} classifier
\citep{Bensmail:1996fw}.
...
similar to that of \cite{Ye:2006tq}. We establish that the matrix operations
...
eigenvalues of the covariance-matrix estimators. Such techniques include work
from \cite{Srivastava:2007ww}, \cite{Rao:1971ul}, and other methods studied by
...
{\tt c4.4xlarge} instance. Our analyses can be reproduced using the code
available in the \url{https://github.com/ramhiser/paper-hdrda} repository.
