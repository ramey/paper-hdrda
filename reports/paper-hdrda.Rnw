...
especially as $p$ increases. The variance in the \emph{RDA} classifier's runtime
also appeared to increase as $p$ increased. To quantify how much beyond linear
growth in $p$, we fit a regression line quadratic in $p$ to the \emph{RDA}
runtimes and plotted the fit in Figure \ref{fig:timing-results}. We attempted to
fit a cubic polynomial in $p$ to the \emph{RDA} timings, but the cubic term was
