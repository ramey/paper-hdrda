...
\citep{Bensmail:1996fw}. Next, applying a reasoning similar to \cite{Ye:2006tq},
we establish that the matrix operations corresponding to the null space of the
pooled sample covariance matrix are redundant and can be discarded from the
...
classifier yields a substantial reduction in computation during model
selection. Also, as a result, we show that a large number of shrinkage methods
that are special cases of the \emph{HDRDA} classifier have no effect on the zero
...
has concluded that the random forest is often superior to other classifiers in
benchmark studies.
...
$\epsilon = 1 \times 10^{-6}$. After calculating $q$, we extract $\bm U_1$ by
retaining only the first $q$ columns of $\bm U$.
...
random-forest classifier from \cite{Breiman:2001fb} as a benchmark. Our
competitors include shrunken centroids regularized discriminant analysis from
\cite{Guo:2007te} and penalized linear discriminant analysis from
\cite{Witten:2011kc}. We also consider two modifications of diagonal linear
discriminant analysis from \cite{Tong:2012hw} and \cite{Pang:2009ik}, where the
former employs an improved mean estimator and the latter utilizes an improved
variance estimator. Below, we refer to each classifier by the first author's
surname.\footnote{TODO: Consider adding a heatmap of error rates of a single
  $(\lambda, \gamma)$ grid.}

We used version 3.2.2 of the open-source statistical software {\tt R} for the
classification study. We used the implementations of the Witten and Guo
classifiers from the {\tt penalizedLDA} and {\tt rda} packages, respectively.
Because the {\tt rda} package does not perform the authors' ``Min-Min'' rule
automatically, we applied this rule within our {\tt R} code. We also utilized
the implementations of the Pang and Tong classifiers available in the {\tt
  sparsediscrim} package. We used the implementation of the random-forest
classifier from the {\tt randomForest} package. For each classifer we explicitly
set prior probabilities as equal if applicable. All other classifier options
were set to their default settings.

For the \emph{HDRDA} classifier in \eqref{eq:hdrda-decomposed}, we examine the
...
\eqref{eq:hdrda-cov} employed resembled \eqref{eq:ridge-estimator}. We estimated
...
We compared the \emph{HDRDA} classifer to the five competing classifiers in a
simulaton study. We simulated $K = 3$ $p$-dimensional multivariate normal
populations with a common positive-definite covariance matrix having a
block-diagonal covariance structure, which is intended to resemble
gene-expression data. That is, within each block of pathways, genes are
positively correlated and that this correlation decays as a function of the
distance between any two genes. Thus, we simulated $K = 3$ multivariate normal
populations with the common positive-definite covariance matrix

\begin{align*}
  \bm\Sigma = \bm\Sigma^{(\rho)} \oplus \bm\Sigma^{(-\rho)} \oplus \ldots \oplus \bm\Sigma^{(\rho)},
\end{align*}

where the $(i,j)$th entry of $\bm\Sigma^{(\rho)} \in \mathbb{R}_{25 \times 25}$ is

\begin{align*}
\bm\Sigma_{ij}^{(\rho)} = \{ \rho^{|i - j|} \}_{1 \le i,j \le 25},
\end{align*}

altering positive and negative values of $\rho$ are used from block to block. In
our simulation study, we considered three different values of $\rho$, 0.1, 0.5,
and 0.9, to explore the effect of autocorrelation on classification accuracy,
especially given that the classifiers from \cite{Tong:2012hw} and
\cite{Pang:2009ik} assume independence among features.

Furthermore, we set the population of mean of class 1 to be a $p$-dimensional
vector of zeros. Next, we set class 2's population mean to a vector equaling 1/2
for the first 50 features and 0 after ($\mu_j = 1/2$ if $j \le 50$ and $\mu_j =
0$ if $j > 50$). Similarly, we set class 3's population mean to a vector equaling -1/2
for the first 50 features and 0 after ($\mu_j = -1/2$ if $j \le 50$ and $\mu_j =
0$ if $j > 50$). The three populations differ in their mean vectors in the first
50 features corresponding to the first two blocks. No difference in the means
occurs in the remaining blocks.

The block-diagonal simulation design considered is widely used within the
high-dimensional classification literature, including the studies by
\cite{Ramey:2013ji}, \cite{Witten:2011kc}, and \cite{Guo:2007te}. While the
design is indeed standard, the simulation configuration is based on an ideal
scenario where no artifacts, often observed in real data, are present. As a
result, we wished to investigate the effect of outliers on the high-dimensional
classifiers. To do this, we generalized the block-diagonal simulation
configuration by sampling from a $p$-dimensional multivariate
$\epsilon$-contaminated normal distribution, where $\epsilon in [0, 1]$. For
$\epsilon = 0$, we have the benchmark block-diagonal simulations considered in
the literature. As $\epsilon$ is increased, the number of outliers is increased
(on average). We consider the values of $\epsilon = 0, 0.05, 0.1, 0.15,
0.2$. For the contamination simulation, we define the probability density
function of the $k$th class to be

\begin{align*}
  p(\bm x | \omega_k) = (1 - \epsilon) N_p(\bm x | \bm \mu_k, \bm \Sigma) + \epsilon N_p(\bm x | \bm \mu_k, 100 * \bm \Sigma)
\end{align*}

From each of the $K=3$ populations, we sampled 25 training observations ($n_k =
25$ for all $k$) and 1000 test observations. After training each classifier on
...
each classifier. Repeating this process 250 times, we computed the average of
the error-rate estimates for each classifier. We allowed the number of features
to vary from $p = 100$ to $p = 500$ in increments of 100 to examine the increase
in dimension while maintaining a small sample size.
