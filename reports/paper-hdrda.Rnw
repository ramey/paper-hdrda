...
average of the error-rate estimates for each classifier.

We used version 3.1.0 of the open-source statistical software {\tt R} for the
...
classifier allowed the option. For the \emph{HDRDA} classifier in
...
inclusively. Here, we provide a concise description of each high-dimensional
