...
We develop a new and computationally efficient regularized discriminant analysis (RDA)
classifier for the small-sample, high-dimensional setting.  We are able
...
class. Moreover, our classifier also differs from
\citeauthor{Friedman:1989tm}'s in that we show our classifier is equivalent to
a classifier in a reduced feature space with dimension approximately equal to the
...
well-suited for high-dimensional classification in practice. We demonstrate that HDRDA is often superior to several
sparse and regularized classifiers in terms of classification accuracy with
three artificial and six real high-dimensional data sets.  Also, timing comparisons
...
\citep{Hastie:2008dt}. Then, we selected the optimal model that minimized the 10-fold
cross-validation error rate.

Here, our goal was to compare the computational runtime of the model selection
for both classifiers as a function of $p$. We compared the
...
reproduced with the code available in the
\url{https://github.com/ramhiser/paper-hdrda} repository.

\subsection{Timing Comparison Results}

In Figure \ref{fig:timing-results}, we plotted the runtime of the model selections
for both the \emph{HDRDA} and \emph{RDA} classifiers as a function of $p$. In the left panel of Figure \ref{fig:timing-results}, we
observe that the \emph{HDRDA} classifier is substantially faster than the
\emph{RDA} classifier as $p$ increases. Figure \ref{fig:timing-results} suggests
that the usage of a matrix inverse and determinant in the {\tt klaR} R package's
discriminant function yielded training and model-selection timings that exceeded
linear growth in $p$. Because our implementation in the \emph{HDRDA} classifier
removes the inverse and determinants from the classifier, the \emph{HDRDA}
classifier is computationally more efficient than the \emph{RDA} classifier,
especially as $p$ increases. We contrast our implementation to the the poor
computational performance of the in the {\tt klaR} R package, which took
approximately 25 minutes on average to train the \emph{RDA} classifier on a data
set having 5,000 features. To quantify how much beyond linear growth in $p$, we
fit a regression line quadratic in $p$ to the \emph{RDA} runtimes and plotted
the fit in Figure \ref{fig:timing-results}. We displayed the
quadratic fit in Figure \ref{fig:timing-results} because it fit the timings best
for $p = 500$ to $p = 2000$. The fits for $2000 \le p \le 5000$ were similar.

Clearly, the model training and selection employed in \emph{HDRDA} is
substantially faster than that of \emph{RDA}. To investigate the relative timing
comparisons between the two classifiers, we considered the ratio of mean timings
of the \emph{RDA} classifier to the \emph{HDRDA} classifier for each value of
$p$. We employed nonparametric bootstrapping\footnote{TODO: Citation needed} to
estimate the mean ratio along with 95\% confidence intervals. In Figure
\ref{fig:timing-comparison-bootstrap}, the bootstrap sampling distributions for
the ratio of mean timings are given. First, we observe that the mean relative
timings increased as $p$ increased. For smaller dimensions, the relative
difference in computing was sizable with the average ratio of the mean timings
equal to \Sexpr{bootstrap_summary_p500$avg_ratio} for $p = 500$ and a 95\%
confidence interval of (\Sexpr{bootstrap_summary_p500$lower_ci},
...
on data sets having 5,000 features in three seconds on average. We contrast our
...
would certainly improve. Moreover, if $q_L = 2$ or $3$ for low-dimensional graphical displays of high-dimensional data can
be obtained.
