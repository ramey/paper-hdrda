...
\cite{Ramey:2013ji} and \cite{Xu:2009fl}.
...
define $\bm X = [\sqrt{c_{1k}(1)} \bm x_1^{T}, \ldots, \sqrt{c_{Nk}(1)} \bm
  x_N^{T}]^{T}$ such that $\widehat{\bm \Sigma} = \bm X^{T} \bm X$. Similarly,
for $\lambda = 0$, we define $\bm X_k = [\sqrt{c_{1k}(0)} \bm x_1^{T}, \ldots,
  \sqrt{c_{Nk}(0)} \bm x_N^{T}]^{T}$ such that $\widehat{\bm \Sigma}_k = \bm
...
\eqref{eq:qda}.\footnote{TODO: Mention the values of $\alpha_k$ and $\gamma$
  needed to resemble HDRDA.}

\subsubsection{Ridge-Like Estimators}

TODO: List a few estimators here along with $\alpha_k = 1$ such that HDRDA
includes them all. For example, set $\gamma = \text{trace}(\widehat{\bm
  \Sigma}_k) / \min(n_k, p)$ for Srivastava and Kubokawa.
...
nonsingular. This assumption is not met when $(\lambda, \gamma) = (0, 0)$ as
$\bm \Gamma_k = \bm 0_q$, corresponding to the \emph{QDA} classifier. In this
special case we apply a small amount of eigenvalue shrinkage and set $\gamma =
0.01$ to circumvent the singularity issue. Also, notice that if $\alpha_k$ is
...
classifier by the first author's surname.\footnote{TODO: Consider adding a
  heatmap of error rates of a single $(\lambda, \gamma)$ grid.}
