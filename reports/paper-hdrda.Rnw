...
$\gamma$ parameters for both classifiers using a grid of 5 equidistant candidate
values between 0 and 1, inclusively. We set $\alpha_k = 1 - \gamma$, $k = 1,
\ldots, K$, in the \emph{HDRDA} classifier to resemble
\citeauthor{Friedman:1989tm}'s \emph{RDA} parameterization. At each pair of
$\lambda$ and $\gamma$, we compute the 10-fold cross-validation error rate
\citep{Hastie:2008dt}. Then, we select the optimal model that minimizes the 10-fold
cross-validation error rate.

Here, our goal is to compare the computational runtime of the model selection
for both classifiers as a function of the number of features $p$. We compare the
runtime of both classifiers by increasing the number of features from $p = 500$
to $p = 5000$ in increments of 500. We generate 100 data sets for each value of
$p$ and compute the training and model selection runtime of both
...
In Figure \ref{fig:timing-results}, we plot the runtime of the model selections
for both the \emph{HDRDA} and \emph{RDA} classifiers as a function of the number
of features $p$. In the left panel of Figure \ref{fig:timing-results}, we
...
the fit in Figure \ref{fig:timing-results}. We attempted to fit a cubic
polynomial in $p$ to the \emph{RDA} timings, but the cubic term was
insignificant with significant quadratic and linear terms. We also fit two
polynomials to the \emph{RDA} timings with orders 2.807 and 2.376, which
correspond to the asymptotic computational runtime of two well-known algorithms
for computing a matrix inverse: the Strassen and Coppersmith-Winograd
algorithms, respectively \footnote{TODO: Citations needed}. In both cases, the
polynomials were statistically significiant at the 0.05 level. We displayed the
...
In the left panel of Figure \ref{fig:timing-results}, we also fit a linear
regression model that was linear in $p$ to the \emph{HDRDA} runtimes. In the
right panel, we repeated the same scatterplot and linear fit with the timings
restricted to the observed range of \emph{HDRDA} timings. The outliers in the
\emph{HDRDA} timings were primarily due to the inherent computing variability in
our {\tt c4.4xlarge} instance.
...
comparisons between the two classifiers, we consider the ratio of mean timings
...
timings increases as $p$ increases. For smaller dimensions, the relative
difference in computing is sizable with the average ratio of the mean timings
...
computing times suggests that the \emph{RDA} classifier is impractical for
higher dimensions. For instance, when $p = 5000$ the ratio of mean computing
times increases to \Sexpr{bootstrap_summary_p5000$avg_ratio} with a 95\%
...
\emph{RDA} classifier on a data set having 5,000 features. Given the \emph{RDA}
