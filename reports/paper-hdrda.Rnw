...
LDA and QDA suck. Friedman to the rescue with RDA. We reformulate the RDA
classifier. Our parameterizaton yields 1) improved interpretation, 2) nice
properties that facilitate dimension reduction, and 3) as a result of the
dimension reduction, efficient calculation for high-dimensional data.

Here, we propose a new regularized discriminant analysis classifier that
incorporates dimension reduction and covariance-matrix shrinkage to enable a
computationally efficient classifier. We demonstrate with four high-dimensional
...
Discriminant analysis is a widely used technique to classify an unlabeled
$p$-dimensional vector $\bm x$ to one of $K$ unique, known classes by
constructing a classifier from $N$ training observations. This is useful in a
variety of areas, such as medical research, facial recognition, and TODO
\cite{Dudoit:2002ev,TODO1,TODO2} because data-driven decision are becoming
increasingly common.  Although a large number of classification algorithms have
been proposed, linear discriminant analysis (LDA) and quadratic discriminant
analysis (QDA) remain popular in practice because of their simplicity, speed of
computation, and interpretability. Both the LDA and QDA classifiers are based on
the assumption that each of the $K$ classes is multivariate normally
distributed. While both methods are popular in practice, they often cannot be
applied to modern data sets because the sample covariance matrices employed are
singular when the number of features exceeds the total sample size $N$. For
instance, standard benchmark microarray data sets used in biological research
often consist of between $p = 10,000$ and $55,000$ probes and fewer than $N =
100$ observations, in which case LDA and QDA are incalculable.\footnote{TODO:
  Bring back the curse of dimensionality reference.}

As a result, several researchers have applied covariance-matrix regularization
to the LDA and QDA classifiers. Covariance-matrix regularization methods often
are based on the well-known ridge-regression approach of \cite{Hoerl:1970cd} and
adjust the eigenvalues of the sample covariance matrices to ensure positive
definiteness.  \cite*{Zhang:2010va}, \cite{Ye:2009gd}, \cite{Ji:2008wp},
...
of the population covariance matrices before applying either the LDA or QDA
classifiers. Alternatively, many researchers, including \cite{Mai:2012bf},
...
matrices to reduce the number of parameters that must be estimated. For
instance, \cite{Dudoit:2002ev} have modified the linear and quadratic
discriminant analysis classifiers by assuming that the population covariance
matrices are diagonal, resulting in classifiers that are special cases of the
Naive Bayes classifier \citep[Chapter 4]{Hastie:2008dt}. For excellent overviews
of the regularization problem in discriminant analysis, see \cite{Murphy:2012uq}
and \cite{Mkhadri:1997gy}.

In a classic paper, \cite{Friedman:1989tm} proposed regularized discriminant
analysis (RDA), which partially pools the individual sample covariance matrices
used in the QDA classifier with the pooled sample covariance matrix from the LDA
classifier. After pooling the covariance matrices, the RDA classifier employs a
covariance-matrix regularization technique that shrinks the zero eigenvalues of
the resulting covariance matrices to ensure they are nonsingular. Given that the
assumption of equal population covariance matrices for the LDA classifier is
often too stringent in practice \citep{Clemmensen:2011kr}, the RDA classifier
can relax the linearity assumption by instead incorporating unequal covariance
matrices while retaining as special cases the LDA and QDA classifiers. Despite
its success, the RDA classifier is often impractical because the model-selection
process for its two tuning parameters requires excessive computation. Moreover,
the pooling parameter has no clear intrepretation, and others
complaints\footnote{TODO: Add some more complaints.}\footnote{TODO: Citations
  needed.}.

We consider the RDA classifier in an attempt to study rigorously some of its
properties. Similar to \cite{Hastie:2008dt} and \cite{Halbe:2007ea}, we first
reparameterize the RDA classifier's covariance-matrix estimator as a convex
combination of the covariance-matrix estimators employed in the LDA and QDA
classifiers. Our parameterizaton yields 1) improved interpretation, 2) nice
properties that facilitate dimension reduction, and 3) as a result of the
dimension reduction, efficient calculation for high-dimensional data. We then
establish the high-dimensional RDA (\emph{HDRDA}) classifier by applying
...
\cite{Xu:2009fl}. This regularization has the effect of stabilizing the
eigenvalues of the covariance-matrix estimators and ensuring that the classifier
is well-posed.
...
matrices, where $n_k$ is the sample size of the $k$th class, $k = 1, \ldots, K$.
This reduction in calculation is clearly advantageous because $n_k$ can be small
in modern high-dimensional data sets (e.g., 10--20 observations in
gene-expression microarray data).

We also demonstrate that the \emph{HDRDA} classifier is often superior to
several recent classifiers designed for small-sample, high-dimensional data. In
particular, we compare our proposed classifier with diagonal linear discriminant
analysis proposed by \cite{Dudoit:2002ev} and the recently proposed classifiers
from \cite{Guo:2007te}, \cite{Pang:2009ik}, \cite{Tong:2012hw}, and
\cite{Witten:2011kc}. We conduct our comparison of the classification accuracy
of the competing classifiers using four small-sample, high-dimensional benchmark
...
discriminant analysis, we require the following notation. Let $\mathbb{R}_{a
  \times b}$ denotes the matrix space of all $a \times b$ matrices over the real
...
respectively. Let $\mathbb{R}_{p \times p}^{>}$ denote the cone of real $p
\times p$ positive-definite matrices. Similarly, let $\mathbb{R}_{p \times
  p}^{\ge}$ denote the cone of real $p \times p$ positive-semidefinite
matrices. Let $V^{\perp} = \{\bm y \in \mathbb{R}_{p \times 1} : \bm y^{T} \bm v
= 0$ for all $\bm v \in V \}$ denote the orthogonal complement of a vector space
$V \subset \mathbb{R}_{p \times 1}$. For $c \in \mathbb{R}$, let $c^+ = 1/c$ if
$c \ne 0$ and $0$ otherwise.
...
\cite{Halbe:2007ea} demonstrate that  parameterization results in a
...
option was available. For the \emph{HDRDA} classifier
\eqref{eq:hdrda-decomposed}, we set $\alpha_k = 1$, $k = 1, \ldots, K$, so that
the covariance-matrix estimator \eqref{eq:hdrda-cov} employed resembled
...
cross-validation \citep{Hastie:2008dt}. Each of the microarray data sets listed
below can be found in \ref{suppA}.
