...
resulting in an interpretable estimator that \emph{borrows} from the pooled
sample covariance matrix. Moreover, we show that \emph{HDRDA} is equivalent to
a classifier in a reduced-feature space with dimension approximately equal to
the training sample size. As a result, the matrix operations employed by
...
\emph{HDRDA} is drastically smaller than that of \emph{RDA}.
...
that the \emph{HDRDA} classifier is \Sexpr{bootstrap_summary_p5000$avg_ratio}
times faster on average than the \emph{RDA} classifier. In this scenario, the
\emph{HDRDA} classifier's model selection requires \Sexpr{hdrda_mean_time_p5000}
seconds on average, while that of the \emph{RDA} classifier requires
\Sexpr{rda_mean_time_p5000} minutes on average.
...
\Sexpr{rda_mean_time_p5000} minutes on average to perform model selection, while
