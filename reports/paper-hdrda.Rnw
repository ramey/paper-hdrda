...
% TODO: Combine first two paragraphs -- list R package along with authors

In this section, we compare our proposed classifier with four classifiers
recently proposed for small-sample, high-dimensional data. Our
...
We used the implementations of the Witten and Guo
...
automatically, we applied this rule within our {\tt R} code. 



We also consider two modifications of diagonal linear discriminant analysis from
\cite{Tong:2012hw} and \cite{Pang:2009ik}, where the former employs an improved
mean estimator and the latter utilizes an improved variance estimator. We also
utilized the implementations of the Pang and Tong classifiers available in the
{\tt sparsediscrim} package.


We also included the random forest from \cite{Breiman:2001fb} as a benchmark
based on the findings of \cite{FernandezDelgado:2014ul}, who has concluded that
the random forest is often superior to other classifiers in benchmark studies.
We used the implementation of the random-forest classifier from the {\tt
  randomForest} package with 250 trees and 100 maximum nodes.

For each classifer we explicitly set prior probabilities as equal if
applicable. All other classifier options were set to their default settings.
We used version 3.2.2 of the open-source statistical software {\tt R} for the
classification study.  Below, we refer to each classifier by the first author's
surname.
