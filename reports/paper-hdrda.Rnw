...
autocorrelation $\rho_k$ alternates between a positive and a negative value from
block to block. The original design from \cite{Guo:2007te} comprised two
$p$-dimensional multivariate normal populations with a common block-diagonal
covariance matrix where $\bm\Sigma_1 = \bm\Sigma_2$.
...
that their dimension reduction inherent better captured the classificatory
...
Standard deviations of the test error rates are given in the parentheses. The
\emph{HDRDA} and Guo classifiers were superior in classification performance for
the majority of the simulations with the popular Guo classifier being the best
for the Burczysnki data set. Both \emph{HDRDA} methods yielded comparable
results.
