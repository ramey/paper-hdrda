...
Notice that if $\alpha_k$ is constant across the $K$ classes, then $\bm
\Gamma_k$ in \eqref{eq:Gamma_k} is independent of $k$. Consequently, $|\bm
\Gamma_k|$ is constant across the $K$ classes and need not be calculated in
\eqref{eq:hdrda-decomposed}.
