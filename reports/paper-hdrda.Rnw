...
The popular regularized discriminant analysis (RDA) classifier proposed by
\citeauthor{Friedman:1989tm} utilizes a biased covariance-matrix estimator that
...
data sets and four high-dimensional real data sets that our proposed classifier
...
analysis \citep{TODO}. Despite the advances in classification to facilitate
technological advancements, well-established classifiers often degrade in
accuracy with modern, high-dimensional data sets when the number of features $p$
exceeds the total sample size $N$, due to the \emph{curse of dimensionality}
\citep{Bellman:1961tn}. For instance, linear discriminant analysis (LDA) and
quadratic discriminant analysis (QDA) are popular because of their simplicity,
speed of computation, and interpretability. However, both methods are
incalculable when $p > N$ because the class sample covariance matrices are
singular \citep*{Bouveyron:2007gx}.
...
parsimony. Covariance-matrix regularization methods often are based on the
...
definiteness. Alternatively, dimension reduction typically decomposes a model to
orthogonal components and retains the components corresponding to the model's
intrinsic dimension. While some classificatory information may be lost,
dimension reduction methods have been shown to be effective in practice
\citep{YePaper,TODO}. Finally, \cite{Mai:2012bf}, \cite{Fan:2012iq},
\cite{Merchante:2012vk}, \cite{Clemmensen:2011kr}, \cite{Huang:2010ju}, and
\cite{Dudoit:2002ev} have proposed restrictions on the population covariance
matrices to reduce the number of parameters that must be estimated. For
instance, \cite{Dudoit:2002ev} have assumed the features within each class are
independent so that the population covariance matrices are diagonal, resulting
in positive-definite sample covariance matrices. For excellent overviews of the
regularization problem in discriminant analysis and other techniques, see
\cite{Murphy:2012uq} and \cite{Mkhadri:1997gy}.
...
covariance matrices for the \emph{LDA} classifier is often too stringent in practice
\citep{Clemmensen:2011kr}, the \emph{RDA} classifier can relax the linearity
assumption by instead incorporating unequal covariance matrices while retaining
as special cases the \emph{LDA} and QDA classifiers. Covariance-matrix
regularization is then applied to the partially pooled covariance-matrix
estimator by shrinking it towards a scaled identity matrix to ensure positive
definiteness. We argue that the \emph{RDA} classifier inherently employs
dimension reduction in addition to regularization. After parameterizing the
\emph{RDA} classifier similar to \cite{Hastie:2008dt} and \cite{Halbe:2007ea},
we demonstrate that the intrinsic dimension of the \emph{RDA} classifier is
equal to the training sample size of each class.\footnote{TODO: Introduce HDRDA
  somewhere.} We then show that the \emph{RDA} decision rule is invariant to
adjustments of the zero eigenvalues. We show that our proposed classifier is
invariant to any adjustments of the eigenvalues corresponding to the null space
of the pooled sample covariance matrix. Applying a reasoning similar to
\cite{Ye:2006tq}, we decompose the decision rule of the \emph{HDRDA} classifier
and show that the sum related to the null space of the pooled sample covariance
matrix can be discarded. Hence, no gain in classification performance can be
obtained by adjusting these zero eigenvalues. This invariance property provides
a substantial increase in computational efficiency of model training and
selection. Moreover, the decision rule in the original feature space is
equivalent to a decision rule in a much lower dimension (i.e., the intrinsic
dimension), such that the matrix inverses and determinants can be rapidly
computed. This invariance property as well as the dual optimization problem
result in substantial reduction in computation. Additionally, our
parameterization of the \emph{RDA} classifier offers interpretability that has
been previously lacking \citep{Bensmail:1996fw}. We explicitly show that the
pooling parameter in the \emph{RDA} covariance-matrix estimator controls the
contribution of each training observation to the estimation of each class
covariance matrix. Furthermore, our parameterization includes a large family of
shrinkage methods, such as those from \cite{Srivastava:2007ww},
...
\emph{RDA} classifier to be employed with high-dimensional data in
...
After demonstrating that the \emph{HDRDA} classifier can be computed efficiently
and establishing its improved interpretability, we compare the computational
runtime of the \emph{HDRDA} classifier with an implementation of the \emph{RDA}
classifier from the {\tt klaR} R package to illustrate that the \emph{HDRDA}
classifier greatly reduces computation when $p \gg N$. We then study the
classification performance of the \emph{HDRDA} classifier on several artificial
and real high-dimensional data sets. We compare the \emph{HDRDA} classifier with
several recent several recent classifiers from \cite{Dudoit:2002ev},
\cite{Guo:2007te}, \cite{Pang:2009ik}, \cite{Tong:2012hw}, and
\cite{Witten:2011kc}, each of which is designed for small-sample,
high-dimensional data. We demonstrate that the \emph{HDRDA} classifier often
attains superior classification accuracy. We also include in our classification
study a support vector machine (\emph{SVM}) and $k$-nearest neigbors ($kNN$) as
benchmarks.

The remainder of this document is organized as follows. In Section
\ref{sec:preliminaries} we presents the discrimination problem and the necessary
notation to describe our contributions. In Section \ref{sec:rda} we present our
