...
result, the dimension reduction involved yields a substantial reduction in
computation during model selection. Our parameterization also offers
interpretability previously lacking with the RDA classifier. We demonstrate with
three artificial and four real high-dimensional data sets that our proposed
classifier is often superior to several recently proposed sparse and regularized
classifiers in terms of classification accuracy. Finally, we provide an
implementation of our proposed classifier in the {\tt sparsediscrim} R package,
available on CRAN.
...
discriminant analysis (\emph{LDA}) and quadratic discriminant analysis
...
similar to \cite*{Hastie:2008dt} and \cite{Halbe:2007ea}, we first show that the
...
space is equivalent to a decision rule in a lower dimension, such that matrix
inverses and determinants of relatively small matrices can be rapidly computed.
Furthermore, we show that several shrinkage methods that are special cases of
the \emph{HDRDA} classifier have no effect on the zero eigenvalues of the
covariance-matrix estimators. Such methods include work from
...
sets along with a simulation design that generalizes the experiments initially
...
\cite{Witten:2011kc}, \cite*{Pang:2009ik}, and \cite{Guo:2007te}. We also
include as a benchmark the random forest from \cite{Breiman:2001fb} because
...
to stabilize the inverse of \eqref{eq:sig-lambda}, \cite{Friedman:1989tm} has
...
= 1, \ldots, K$, to resemble \citeauthor{Friedman:1989tm}'s parameterization. We
estimated both $\lambda$ and $\gamma$ from a grid of 21 equidistant candidate
values between 0 and 1, inclusively.
...
outliers is increased. In our simulation, we set $\eta = 100$ and considered the
...
all $k$. To explore the more realistic assumption of unequal covariance
matrices, we instead put $\rho_1 = 0.1$, $\rho_2 = 0.5$, and $\rho_3 = 0.9$
...
rates for these three classifiers continued as $\epsilon$ increased
further. Guo's method always outperformed Pang, Witten, and Tong, but once
outliers were present, the Guo classifier's average error rate was competitive
with neither the \emph{HDRDA} classifiers nor the random forest.
...
the majority of the simulations with the popular Guo classifier providing
superior classification performance for the Burczysnki data set. Both
\emph{HDRDA} methods yielded comparable results.
...
Pang and Tong classifiers were approximately equal throughout our study,
suggesting that the mean and variance estimators used in lieu of the MLEs
provided little improvement to classification accuracies. The random-forest
classifier was competitive only on the Singh data set. This result challenges
the claim of \cite{FernandezDelgado:2014ul} that the random forest is
typically superior. More studies should be performed to validate this statement
in the small-sample, high-dimensional setting.

Finally, the average test error rates for the Witten classifier were the largest
across all data sets. Given that the standard deviations were relatively large,
we hesitate to generalize claims regarding the Witten classifier being the
uniformly worst performer. However, the variability in its classification
accuracy across multiple random partitions of each data set were large enough to
rethink applying it in practice.
