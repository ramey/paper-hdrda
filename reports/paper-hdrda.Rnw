...
$\widehat{\bm \Sigma} = \bm X^{T} \bm X$\footnote{TODO: $1/N$ is missing. Add
  this. How much do things change below?} Then, because $\bm X = \bm M \bm
...
\begin{algorithm}
  ...
  \Output{Optimal Estimates, $\hat{\lambda}$ and $\hat{\gamma}$}
  \BlankLine
  \For{$v \leftarrow 1$ \KwTo $V$ \tcp*[r]{$V$-fold cross-validation}} {
    Partition $v$th fold into $\bm X_{train} \in \mathbb{R}_{N \times p}$ and
    ...
    \For{$k\leftarrow 1$ \KwTo $K$}{
      ...
      Center $\bm X_k \leftarrow \bm X_k - \bm j_{n_k} \xbar_k^T$    
    }
  ...
  Report optimal $(\hat{\lambda}, \hat{\gamma}) \leftarrow \argmax_{(\lambda, \gamma)} \widehat{\text{Error}}(\lambda, \gamma)$
  \caption{Model selection for the \emph{HDRDA} classifier}
  \label{algo_hdrda_model_selection}
\end{algorithm}
\DecMargin{1em}

The compact SVD is sometimes called the reduced or fast SVD.


Let $\bm j_m \in \mathbb{R}_{m \times 1}$ denote a vector of ones.
