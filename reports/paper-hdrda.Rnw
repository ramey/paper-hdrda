...
\gg N$ \cite[Chapter~18.3.5]{Hastie:2008dt}. Recall that the pooled sample
covariance matrix is $\widehat{\bm \Sigma} = \bm X^{T} \bm X$\footnote{TODO:
  $1/N$ is missing. Add this. It will be absorbed into the eigenvalues.}  Then,
because $\bm X = \bm M \bm D^{1/2} \bm U^{T}$ by the SVD, we have the
...
set by calculating $\bm X_c \bm U_1$ once per fold. Conveniently, we see that
the most costly computation involved in $\bm Q_k$ and $\bm W_k^{-1}$ is $\bm X_k
\bm U_1$, which can be extracted from $\bm X_c \bm U_1$. Thus, after the initial
calculation of $\bm X_c \bm U_1$ per cross-validation fold, $\bm Q_k$ requires
$O(TODO)$ operations. Because $\bm Q_k \in \mathbb{R}_{n_k \times n_k}$, both
its determinant and inverse requires $O(n_k^3)$ operations. This is especially
advantageous when $K$ is large, such that $n_k$ is much smaller than $N$. Thus,
$\bm W_k^{-1}$ requires $O(TODO)$ operations Also, the inverse and determinant
of the diagonal matrix $\bm \Gamma_k \in \mathbb{R}_{q \times q}$ requires
$O(q)$ operations. Hence, the $| \bm W_k |$ requires $O(q n_k^3)$ operations,
which is efficient when $p >> N$.
...
and $\gamma$.\footnote{TODO: Need to clean up this paragraph so that it flows
  from above. Also, elaborate.}
...
$V$-fold cross-validation with $V = 10$ \citep{Hastie:2008dt}. For the second
model, we set $\alpha_k = 1 - \gamma$, $k = 1, \ldots, K$, to resemble
