...
Reliable supervised classification of tumors from gene expression microarray
...
efficient classifier. We demonstrate with four high-dimensional microarray data
sets that our proposed classifier is often superior to several recently proposed
sparse and regularized classifiers in terms of classification accuracy. Finally,
we provide an implementation of our proposed classifier in the {\tt
...
\kwd{high-dimensional multivariate analysis}
\kwd{microarray data}
\end{keyword}
...
class. We further assume that each of $p(\omega_k)$ is equal, $k = 1, \ldots, K$.
...
Throughout the remainder of this document we also require the following
additional notation. Let $\bm I_m$ be the $m \times m$ identity matrix, and let
$\bm 0_{m \times p}$ be the $m \times p$ matrix of zeros, such that $\bm 0_m$ is
understood to denote $\bm 0_{m \times m}$. For $c \in \mathbb{R}$, let $c^+ =
1/c$ if $c \ne 0$ and $0$ otherwise. Let $\bm A \oplus \bm B$ denote the direct
sum of $\bm A \in \mathbb{R}_{r \times r}$ and $\bm B \in \mathbb{R}_{s \times
  s}$ \citep[Chapter 1]{Lutkepohl:1996uz}. Let $\bm A^+$ and $\mathcal{N}(\bm A)
= \{\bm z \in \mathbb{R}_{p \times 1} : \bm A \bm z = \bm 0_{m \times 1}\}$
denote the Moore-Penrose pseudoinverse and null space of $\bm A \in
\mathbb{R}_{m \times p}$, respectively. Let $V^{\perp} = \{\bm y \in
\mathbb{R}_{p \times 1} : \bm y^{T} \bm v = 0$ for all $\bm v \in V \}$ denote
...
\slink[url]{https://github.com/ramey/paper-hdrda/tree/annals-of-applied-statistics/reports}
