...
%  Center,\\ Seattle, Washington 98115, U.S.A. \email{johnramey@gmail.com}}
%\affil{Department of Statistical Science, Baylor University, Waco, Texas 76706, U.S.A. \email{Caleb\_Stein@baylor.edu} \email{Dean\_Young@baylor.edu}}
...
cancer; however, because the number of gene expression levels measured is
typically much greater than the sample size, the analysis of microarray data can
be difficult because classic classifiers are often ill-posed in this setting and
require a large number of calculations. To address these challenges, we propose
a new regularized discriminant analysis classifier that incorporates dimension
reduction and covariance-matrix shrinkage to enable an efficacious,
computationally efficient method. We demonstrate with four high-dimensional
microarray data sets that our proposed classifier is often superior to several
recently proposed sparse and regularized classifiers in terms of classification
accuracy. Finally, we provide an implementation of our proposed classifier in
...
predefined types, where our goal is to ascertain the class membership of
...
matrices in linear and quadratic discriminant analysis to reduce the number of
...
discriminant analysis, see \cite{Murphy:2012uq}, \cite{Mkhadri:1997gy}, and
\cite{Friedman:1989tm}.

Here, we propose the high-dimensional regularized discriminant analysis
(\emph{HDRDA}) by first constructing a pooling matrix as a convex combination of
the covariance-matrix estimators employed in linear and quadratic discriminant
analysis. Given that the assumption of equal population covariance matrices for
linear discriminant analysis is often too stringent in practice
\citep{Clemmensen:2011kr}, our proposed classifier can relax the linearity
assumption by instead incorporating unequal covariance matrices while retaining
linear and quadratic discriminant analysis as special cases. We then establish
the \emph{HDRDA} classifier by applying covariance-matrix regularization with a
parameterization that includes a large family of shrinkage methods, such as
those from \cite{Srivastava:2007ww}, \cite{Rao:1971ul}, and other methods
studied by \cite{Ramey:2013ji} and \cite{Xu:2009fl}. This regularization has the
effect of stabilizing the eigenvalues of the covariance-matrix estimators and
ensuring that the classifier is well-posed.
...
decision rule of the \emph{HDRDA} classifier and show that the sum related to
the null space of the pooled sample covariance matrix can be discarded. Hence,
no gain in classification performance can be obtained by adjusting these zero
eigenvalues. This invariance property provides a substantial increase in
computational efficiency of model training and selection. In fact, we show that
the inverse and determinant can be reduced to calculations of $n_k \times n_k$
...
often quite small in microarray data sets (e.g., 10--20 observations).

We also demonstrate that the \emph{HDRDA} classifier is competitive with and
often superior to several recent classifiers designed for small-sample,
high-dimensional data. In particular, we compare our proposed classifier with
diagonal linear discriminant analysis proposed by \cite{Dudoit:2002ev} and the
recently proposed classifiers from \cite{Guo:2007te}, \cite{Pang:2009ik},
...
high-dimensional benchmark microarray data sets.
...
observations. Let $\bm x_i = (x_{i1}, \ldots, x_{ip}) \in \mathbb{R}_{p \times
...
$\mathbb{R}$. We assume that $(\bm x_i, y_i)$ is a realization from a mixture
distribution $p(\bm x) = \sum_{k=1}^K p(\bm x | \omega_k) p(\omega_k)$, where
$p(\bm x | \omega_k)$ is the probability density function of the $k$th class and
...
$\bm \mu_k \in \mathbb{R}_{p \times 1}$ and known covariance matrix $\bm
\Sigma_k \in \mathbb{R}_{p \times p}^{>}$, $k = 1, 2, \ldots, K$, where
$\mathbb{R}_{p \times p}^{>}$ denotes the cone of real $p \times p$
positive-definite matrices. Because $\bm \mu_k$ and $\bm \Sigma_k$ are typically
unknown, we assign an unlabeled observation $\bm x$ to class $\omega_k$ with the
sample quadratic discriminant analysis classifier
\begin{align}
...
estimators of $\bm \mu_k$ and $\bm \Sigma_k$, respectively, and $\bm A^{T}$
denotes the matrix transpose of $\bm A \in \mathbb{R}_{m \times p}$. If we
assume further that $\bm \Sigma_k = \bm \Sigma$ $(k = 1, \ldots, K)$, then the
pooled sample covariance matrix $\widehat{\bm \Sigma}$ is substituted for
$\widehat{ \bm \Sigma}_k$ in \eqref{eq:qda}, where
\begin{align}
...
$\bm I_m$ be the $m \times m$ identity matrix, and let $\bm 0_{m \times p}$ be
the $m \times p$ matrix of zeros, such that $\bm 0_m$ is understood to denote
$\bm 0_{m \times m}$. For $c \in \mathbb{R}$, let $c^+ = 1/c$ if $c \ne 0$ and
$0$ otherwise. Let $\bm A \oplus \bm B$ denote the direct sum of $\bm A \in
\mathbb{R}_{r \times r}$ and $\bm B \in \mathbb{R}_{s \times s}$ \citep[Chapter
  1]{Lutkepohl:1996uz}. Let $\bm A^+$ and $\mathcal{N}(\bm A) = \{\bm z \in
\mathbb{R}_{p \times 1} : \bm A \bm z = \bm 0_{m \times 1}\}$ denote the
Moore-Penrose pseudoinverse and null space of $\bm A \in \mathbb{R}_{m \times
  p}$, respectively. Let $V^{\perp} = \{\bm y \in \mathbb{R}_{p \times 1} : \bm
y^{T} \bm v = 0\ \forall \bm v \in V \}$ denote the orthogonal complement of a
vector space $V \subset \mathbb{R}_{p \times 1}$.

\subsection{Covariance Matrix Regularization}

The smallest eigenvalues of $\widehat{\bm \Sigma}_k$ and the directions
associated with their eigenvectors highly influence \eqref{eq:qda}. In fact, the
eigenvalues of $\widehat{ \bm \Sigma}_k$ are well-known to be biased if $p \ge
n_k$ such that the smallest eigenvalues are underestimated \citep{Seber:2004uh},
Moreover, if $p > n_k$, then rank$(\widehat{\bm \Sigma}_k) \le n_k$, which
implies that at least $p - n_k$ eigenvalues of $\widehat{\bm \Sigma}_k$ are
0. Furthermore, although more feature information is available to discriminate
among the $K$ classes, if $p > n_k$, \eqref{eq:qda} is incalculable because
$\widehat{\bm \Sigma}_k^{-1}$ does not exist.
...
%	&= \left( \frac{1 - \lambda}{n_k} + \frac{\lambda}{N} \right)\sum_{i=1}^N I(y_i = k) x_i x_i^{T} +  \frac{\lambda}{N} \sum_{i=1}^N I(y_i \ne k) x_i x_i^{T} \nonumber \\
...
$N$ rather than $n_k$ observations. Below, we will show that the pooling
...
\emph{shrinkage} parameter $\gamma$ determines the amount of eigenvalue
shrinkage applied. The choice of $\alpha_k$ allows for a flexible formulation of
...
itself to a more efficient calculation. We decompose \eqref{eq:hdrda} into a sum
of two components, where the first summand consists of matrix operations applied
to low-dimensional matrices and the second summand corresponds to the null space
of $\widehat{\bm \Sigma}$ in \eqref{eq:pooled-cov}. We show that the matrix
...
computational costs is achieved. Furthermore, a byproduct is that adjustments to
the associated eigenvalues have no effect on \eqref{eq:hdrda}. Lastly, we
utilize the singular value decomposition to calculate efficiently the eigenvalue
...
\eqref{eq:hdrda} is invariant to adjustments to the eigenvalues of $\tilde{ \bm
...
We now present our main result, where we decompose the decision rule of the
\emph{HDRDA} classifier and show that the term requiring the largest
computational costs does not contribute to the classification of an unlabeled
observation performed using Lemma \ref{lemma:RDA-constant-term}. Hence, we
reduce \eqref{eq:hdrda} to an equivalent, more computationally efficient
decision rule.
...
\mathbb{R}_{p \times 1}$, we have that
	...
	&+ \gamma^{+} (\bm x - \xbar_k)^{T} \bm U_2 \bm U_2^{T} (\bm x - \xbar_k) + \log | \bm W_k | + (p - q) \log \gamma.
	\end{align*}
...
option was available. For the \emph{HDRDA} classifier, we set $\alpha_k = 1$ $(k
= 1, \ldots, K)$, so that the covariance-matrix estimator \eqref{eq:hdrda-cov}
employed resembled \eqref{eq:ridge-estimator}. We estimated $\lambda$ from a
grid of 21 equidistant candidate values between 0 and 1, inclusively. Similarly,
we estimated $\gamma$ from a grid consisting of the values $10^{-1}, \ldots,
10^4$, and $10^5$. We selected optimal estimates of $\lambda$ and $\gamma$ using
...
Nakayama data set, the \emph{HDRDA} classifier was outperformed by the Dudoit,
Pang, and Tong classifiers for small values of $d$ but attained classification
rates superior to its competitors for larger values of $d$. Furthermore, Figure
...
contrast to the other classifiers, the \emph{HDRDA} classifier exhibited less
variability in its misclassification estimates for small values of $d$ and
...
p <- p + geom_line(aes(), size = 1, linetype = 2)
p <- p + geom_point(aes(shape = Classifier), size = 6)
...
\caption{Averages of the 1000 error-rate estimates of the competing classifiers
  for each data set as a function of $d$. The classifiers are marked as follows:
  Dudoit (circle), Guo (triangle), Pang (square), Tong = (cross), Witten (square
  with X), and ours (asterisk). Approximate standard errors are no greater than
  \Sexpr{max_se}.}
...
optimal estimates of $\lambda$ and $\gamma$ in the Supplementary Material. For
each of the four data sets, the distribution of the optimal values of $\lambda$
selected through cross-validation often had a notable bathtub shape, resembling
a Beta distribution with both shape parameters less than unity. Specifically,
the optimal values of $\lambda$ were typically near 0 or near 1, corresponding
to quadratic and linear discriminant analysis, respectively. For instance, with
...
see the flexibility of our proposed classifier to employ the more realistic
assumption that the class covariance matrices are unequal, but if optimal, we
can employ the more parsimonious assumption of equal covariance matrices.

\section{Discussion}

In our simulation study, we compared our proposed \emph{HDRDA} classifier to
several recently proposed classifiers, including the Dudoit, Pang, Tong, and Guo
...
implementation of our classifier in the {\tt sparsediscrim} R package was
comparable to that of the diagonal classifiers in terms of computational
runtime. Moreover, when preliminary variable selection was applied to the data
in our classification study, the time to perform model selection with our
proposed classifier was nearly instantaneous. Hence, our proposed \emph{HDRDA}
classifier is competitive in terms of classification accuracy for modern
small-sample, high-dimensional data sets and requires substantially less
computing time than standard regularized classifiers, such as the \emph{RDA}
classifier proposed by \cite{Friedman:1989tm}.

\section*{Supplementary material}
\label{SM}
Supplementary material available online includes the graphical summary
of the optimal estimates of our classifier from the classification study.
