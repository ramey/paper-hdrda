...
performance of the \emph{HDRDA} classifier on three artificial and four real
high-dimensional data sets. We demonstrate that the \emph{HDRDA} classifier
...
\cite{Witten:2011kc}, \cite{Pang:2009ik}, \cite{Guo:2007te}, and
\cite{Dudoit:2002ev}. We also include as benchmarks support vector machines
(\emph{SVM}) and $k$-nearest neigbors ($k$-NN) in our classification
study.\footnote{TODO: If we change any of the classifiers in the simulations,
