...
\runtitle{High-Dimensional Regularized Discriminant Analysis}

\begin{aug}
\author{\fnms{John A.} \snm{Ramey}\thanksref{m1}
  \ead[label=e1]{johnramey@gmail.com}
  \ead[label=u1,url]{http://ramhiser.com}}
\author{\fnms{Caleb K.} \snm{Stein}\thanksref{m2}\ead[label=e2]{Caleb\_Stein@baylor.edu}}
\and
\author{\fnms{Dean M.} \snm{Young}\thanksref{m2}\ead[label=e3]{Dean\_Young@baylor.edu}}

\runauthor{J. A. Ramey et al.}

\affiliation{Fred Hutchinson Cancer Research Center\thanksmark{m1} and Baylor University\thanksmark{m2}}

\address{Vaccine and Infectious Disease Division\\
...
typically much greater than the sample size, the analysis of microarray data
often presents difficult statistical and computational challenges. Here, we
propose a new regularized discriminant analysis classifier that incorporates
dimension reduction and covariance-matrix shrinkage to enable a computationally
efficient classifier and model-selection procedure. We demonstrate with four
high-dimensional microarray data sets that our proposed classifier is often
...
\begin{keyword}
\kwd{classification}
\kwd{regularized discriminant analysis}
\kwd{covariance-matrix regularization}
\kwd{high-dimensional microarray data}
...
the inverse and determinants can be reduced to calculations of $n_k \times n_k$
...
\eqref{eq:hdrda-cov} into \eqref{eq:qda}, we define the high-dimensional
...
avoids the selection bias discussed in \cite{Ambroise:2002fa}.
...
10-fold cross-validation \citep{Hastie:2008dt}. Each of the microarray data sets
listed below can be found in \ref{suppA}.
...
p <- p + geom_point(aes(color = Classifier, shape = Classifier), size = 4)
...
optimal estimates of $\lambda$ and $\gamma$ in \ref{suppB}. For
...
\stitle{R Package Containing Microarray Data Sets}
\slink[url]{https://github.com/ramey/datamicroarray}
\sdescription{An R package containing all of the microarray data sets used in
  the classification study.}
\end{supplement}

\begin{supplement}
\sname{Supplement B}\label{suppB}
\stitle{Optimal Estimates of the HDRDA Classifier}
\slink[url]{https://github.com/ramey/paper-hdrda}
\sdescription{The graphical summary of the optimal estimates of the \emph{HDRDA} classifier from the classification study.}
\end{supplement}
