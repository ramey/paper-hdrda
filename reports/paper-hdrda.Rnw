...
Witten classifiers. For smaller dimensions, the random-forest and HDRDA
...
both HDRDA classifiers outperformed all other classifiers, suggesting that their
dimension reduction inherent better captured the classificatory information in
the small samples even in the presence of outliers.
...
p <- p + xlab("Contamination Probability") + ylab("Expected Error Rate")
p + theme_bw()
@
...
with neither the HDRDA classifiers nor the random forest.
...
p2 <- p2 + xlab("Number of Features") + ylab("Expected Error Rate")
p2 + theme_bw()
@
...
classifiers. Furthermore, the HDRDA classifiers were superior in terms of
...
performance of the HDRDA and Guo classifiers was hardly affected by $p$.  Also,
as discussed above, the HDRDA classifiers were superior to all other classifiers
for larger values of $p$ with only the random forest outperforming them for
