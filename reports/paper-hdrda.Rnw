...
\eqref{eq:hdrda-decomposed} with a substantial reduction in computational
complexity. Here, we demonstrate that the computational efficiency in
calculating the inverse and determinant of $\bm W_k$ can be further improved via
...
\bm \Gamma_k| |\bm Q_k|$ and
    ...
    \intertext{where}
    \bm Q_k &= \bm I_{n_k} + \alpha_k(1 - \lambda) \bm X_k \bm U_1 \bm \Gamma_k^{-1} \bm U_1^{T} \bm X_k^{T}\label{eq:Q_k}
    \intertext{and}
...
facilitate an efficient calculation of the decision rule. Here, we describe in
an efficient model-selection procedure to select the optimal estimates for the
tuning parameters $\lambda$ and $\gamma$. We provide pseudocode for the model
selection in Algorithm \ref{alg:hdrda_model_selection}. Our goal is to select
the optimal tuning parameter estimates from the Cartesian product of candidate
values, $\{\lambda_g\}_{g=1}^G \times \{\gamma_h\}_{h=1}^H$. We utilize $V$-fold
cross-validation to estimate the conditional error rate for each candidate
tuning-parameter pair and select the pair that attains the minimum error
rate. To calculate the $V$-fold cross-validation, we partition the original
training data into $V$ mutually exclusive and exhaustive folds that have
approximately the same number of observations. Then, for $v = 1, \ldots, V$, we
classify the observations in the $v$th fold by training a classifier on the
remaining $V - 1$ folds for each candidate pair of tuning parameters. We then
calculate the proportion of misclassified observations across the $V$ folds and
select the pair $(\widehat{\lambda}, \widehat{\gamma})$ that minimizes the
considered training error-rate estimator.
...
\begin{algorithm}
  ...
  \For{$v \leftarrow 1$ \KwTo $V$} {
    ...
    Compute $\bm X_c = \bm M_q \bm D_q \bm U_1^T$
    ...
    \For{$(\lambda, \gamma) \in \{\lambda_g\}_{g=1}^G \times \{\gamma_h\}_{h=1}^H$} {
      \For{$k \leftarrow 1$ \KwTo $K$} {      
        ...
        Compute $(\bm x - \xbar_k)^{T} \bm U_1 \bm W_k^{-1} \bm U_1^{T} (\bm x - \xbar_k) + \log | \bm W_k |$ \tcp*[r]{TODO: Use matrix form for test observations}
      }
      Classify test observations $\bm X_{test}$ using \eqref{eq:hdrda-decomposed}
      
      Compute $\#\{\text{Error}_v(\lambda, \gamma)\} \leftarrow$ misclassified test observations
    }   
   }
...
% TODO: Now that the algorithm has been introduced and described, we need to
% discuss the details. Let's remove some of the bloat where we talk about the
% kernel trick, etc. Simply discuss how this technique is fast and cite Hastie's
% book.
...
decomposition (SVD) (sometimes called the reduced or fast SVD), which we
describe here. Rather than computing the eigenvalue decomposition of
...
\gg N$ \citep{Hastie:2008dt}. Recall that the pooled sample covariance matrix is
$\widehat{\bm \Sigma} = \bm X^{T} \bm X$\footnote{TODO: $1/N$ is missing. Add
  this. It will be absorbed into the eigenvalues.}  Then, because $\bm X = \bm M
\bm D^{1/2} \bm U^{T}$ by the SVD, we have the eigendecomposition $\widehat{\bm
  \Sigma} = \bm U \bm D \bm U^{T}$, where $\bm M \in \mathbb{R}_{N \times N}$ is
orthogonal, $\bm D^{1/2} \in \mathbb{R}_{N \times N}^{\ge}$ is a diagonal matrix
consisting of the singular values of $\bm X$, and $\bm U \in \mathbb{R}_{p
  \times N}$ is orthogonal. Now, $\bm X \bm X^{T} = \bm M \bm D \bm M^{T}$ is
the eigenvalue decomposition of $\bm X \bm X^{T}$, where $\bm D$ is the matrix
of eigenvalues of $\bm X \bm X^{T}$ and the columns of $\bm M$ are the
corresponding eigenvectors. Hence, we obtain $\bm M$ and $\bm D$ from an
eigendecomposition of $\bm X \bm X^{T} \in \mathbb{R}_{N \times N}$. Next, we
compute $\bm U = \bm X^{T} \bm M \bm D^{+/2}$, where $\bm D^{+/2} = \bm
D_q^{-1/2} \oplus \bm 0_{N-q}$. We then determine $q$, the number of numerically
nonzero eigenvalues present in $\bm D$, by calculating the number of eigenvalues
that exceeds some tolerance value, say, $\epsilon = 1 \times 10^{-6}$. After
calculating $q$, we extract $\bm U_1$ by retaining only the first $q$ columns of
$\bm U$.

% TODO: Discuss things that need only be computed once
% TODO: Discuss operations independent of tuning parameters
...
calculations to compute both $\bm W_k^{-1}$ and $| \bm W_k |$. The expressions
given in Proposition \ref{proposition:hdrda-W_k} also expedite the selection of
$\lambda$ and $\gamma$ via cross-validation because the most time-consuming
matrix operation involved in computing $\bm W_k^{-1}$ and $|\bm W_k|$ is $\bm
X_k \bm U_1 \in \mathbb{R}_{n_k \times q}$, which is independent of $\lambda$
and $\gamma$. Therefore, we need calculate $\bm X_k \bm U_1$ only once for each
...
\begin{enumerate}
  \item $\bm W_k^{-1}$\footnote{TODO: Need complexity for the following}
  \item $|\bm W_k|$
  \item (12)
\end{enumerate}
...
We used version 3.0.2 of the open-source statistical software {\tt R} for the
