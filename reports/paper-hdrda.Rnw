...
In this section we compare the competing classifiers using the simulation
design from \cite{Guo:2007te}. This design is widely used within the
high-dimensional classification literature including the studies by
\cite{Ramey:2013ji} and \cite{Witten:2011kc}. The original design comprised two
$p$-dimensional multivariate normal populations with the common block-diagonal
covariance matrix
\begin{align}
  \bm\Sigma = \bm\Sigma^{(\rho)} \oplus \bm\Sigma^{(-\rho)} \oplus \ldots \oplus \bm\Sigma^{(\rho)},\label{eq:block-sigma}
\end{align}
where the $(i,j)$th entry of the block matrix $\bm\Sigma^{(\rho)} \in
\mathbb{R}_{25 \times 25}$ is
\begin{align*}
...
autocorrelation $\rho$ alternates between a positive and a negative value from block
to block.
...
multivariate normal distribution by $N_p(\bm x | \bm \mu, \bm \Sigma)$, we
write the PDF of the $k$th class as
\begin{align}
  p(\bm x | \omega_k) = (1 - \epsilon) N_p(\bm x | \bm \mu_k, \bm \Sigma) + \epsilon N_p(\bm x | \bm \mu_k, \omega \bm \Sigma),\label{eq:contaminated-pdf}
\end{align}
where $\epsilon \in [0, 1]$ is the probability that an observation is contaminated (i.e.,
drawn from a distribution with larger variance) and $\omega > 1$ scales the
covariance matrix $\bm \Sigma$ to increase the extremity of outliers. For
$\epsilon = 0$, we have the benchmark block-diagonal simulation design from
\cite{Guo:2007te}. As $\epsilon$ is increased, the average number of outliers
is increased. In our simulation, we considered the values of $\epsilon = 0$,
$0.05$, $0.1$, $0.15$, and $0.2$. We also set $\omega = 100$.

We generated $K=3$ populations from \eqref{eq:contaminated-pdf} using the
covariance matrix $\bm \Sigma$ given in \eqref{eq:block-sigma}. We set the mean
vector of class 1 to $\bm \mu_1 = \bm 0_p$. Next, comparable to
\cite{Guo:2007te}, the first 50 features of $\bm \mu_2$ were 1/2, while the rest
were set to 0, i.e., $\bm \mu_2 = (\underbrace{1/2, \ldots, 1/2}_{50},
\underbrace{0, \ldots, 0}_{p - 50})$. For simplicity, we defined $\bm \mu_3 =
-\bm \mu_2$. The three populations differed in their mean vectors in the first
...
small sample size. Furthermore, although \cite{Guo:2007te} consider $\rho =
0.9$, we considered $\rho = 0.1$, $0.5$, and $0.9$ to explore the effect of
autocorrelation on classification accuracy, especially given that the Tong,
Pang, and Guo classifiers assume independence among features.
