...
\in \mathbb{R}_{p \times 1} : y^{T}v = 0\ \forall v \in V \}$ denote
...
where $D^{+/2} = D_q^{-1/2} \oplus 0_{N-q}$. We then determine $q$ by computing
the number of numerically nonzero eigenvalues present in $D$, where $q$ is the
number of eigenvalues that exceeds some tolerance value, say, $\epsilon = 1
\times 10^{-6}$. After calculating $q$, we extract $U_1$ by retaining only the
...
K)$, so that the covariance-matrix estimator \eqref{eq:hdrda-cov} employed had a
form resembling \eqref{eq:ridge-estimator}. We estimated $\lambda$ from a grid
of 21 equidistant candidate values between 0 and 1, inclusively. Similarly, we
...
p <- p + geom_line(aes(color = Classifier), size = 1.5, linetype = 2)
...
\caption{Averages of the 1000 error-rate estimates of the competing classifiers
  ...
  Dudoit (red circle), Guo (gold triangle), Pang (green square), Tong = (cyan
  cross), Witten (blue square with X), and ours (pink asterisk). Approximate
  standard errors are no greater than $\Sexpr{gsub('.', '\\cdot', round(max_se, 3), fixed = TRUE)}$.}
