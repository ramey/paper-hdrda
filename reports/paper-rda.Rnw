...
density function of the $k$th class and $p(\omega_k)$ is the prior probability
of class membership of the $k$th class. We further assume that each of
$p(\omega_k)$ is equal $(k = 1, \ldots, K)$.
...
$\bm\mu_k \in \mathbb{R}_{p \times 1}$ and known covariance matrix $\bm\Sigma_k
\in \mathbb{R}_{p \times p}^{>}$, $k = 1, 2, \ldots, K$, where $\mathbb{R}_{p
  \times p}^{>}$ denotes the cone of real $p \times p$ positive-definite
matrices. Because $\bm \mu_k$ and $\bm\Sigma_k$ are typically unknown, we assign
an unlabeled observation $\bm x$ to class $\omega_k$ with the sample quadratic
discriminant analysis classifier
\begin{align}
	D_{QDA}(\bm x) = \argmin_{k}  (\bm x - \xbar_k)'\widehat{\bm\Sigma}_k^{-1}(\bm x - \xbar_k)  + \log |\widehat{\bm\Sigma}_k|, \label{eq:qda}
\end{align}
where $\xbar_k$ and $\widehat{\bm\Sigma}_k$ are the maximum likelihood
estimators for $\bm \mu_k$ and $\bm \Sigma_k$, respectively. If we assume
further that $\bm\Sigma_k = \bm\Sigma$ $(k = 1, \ldots, K)$, then the pooled
sample covariance matrix $\widehat{\bm\Sigma}$ is substituted for $\widehat{\bm
  \Sigma}_k$ in \eqref{eq:qda}, where
\begin{align}
	\widehat{\bm\Sigma} = N^{-1} \sum_{k=1}^K n_k \widehat{\bm\Sigma}_k \label{eq:pooled-cov}
\end{align}
is the maximum likelihood estimator for $\bm \Sigma$. In this case
\eqref{eq:qda} reduces to the sample linear discriminant analysis classifier,
and the log-determinant is omitted because it is constant across all classes.
...
$\widehat{\bm\Sigma}_k$ used in \eqref{eq:qda}. Alternatively, if $\lambda = 1$,
...
analysis classifier
\begin{align}
	D_{RDA}(\bm x) = \argmin_{k}  (\bm x - \xbar_k)'\widehat{\bm\Sigma}_k(\lambda, \gamma)^{-1}(\bm x - \xbar_k)  + \log |\widehat{\bm\Sigma}_k(\lambda, \gamma)|. \label{eq:rda}
\end{align}
...
rewriting \eqref{eq:sig-lambda-alternative} in terms of the observations $\bm
...
each of the $N$ observations to estimate $\bm \Sigma_k$. Hence, for $0 < \lambda
\le 1$, we estimate $\bm \Sigma_k$ with $N$ rather than $n_k$
...
that $\widehat{\bm \Sigma}_k = \bm X_k' \bm X_k$. In Figure
...
\begin{align}
	D_{HDRDA}(\bm x) = \argmin_{k}  (\bm x - \xbar_k)'\tilde{\bm\Sigma}_k^{+}(\bm x - \xbar_k)  + \log |\tilde{\bm\Sigma}_k|. \label{eq:hdrda}
\end{align}
...
\eqref{eq:sig-lambda-alternative}. Then, $\mathcal{N}\{\widehat{\bm \Sigma}_k(\lambda)\} \subset \mathcal{N}(\widehat{\bm
  \Sigma}) \subset \mathcal{N}(\widehat{\bm \Sigma}_k)$ $(k = 1, \ldots, K)$.
\end{lemma}
\begin{proof}
...
\widehat{\bm \Sigma} \bm z = N^{-1} \sum_{k = 1}^K n_k \bm z' \widehat{\bm
  ...
  \Sigma}_k)$ because $\widehat{\bm \Sigma}_k \in \mathbb{R}_{p \times
...
q \le p$. Then, we have
\begin{align}
	\tilde{\bm \Sigma}_k &= \bm U(\bm W_k \oplus \gamma \bm I_{p-q})\bm U' \quad
...
\bm 0_{p \times (p - q)}$. Hence, $\bm U' \widehat{\bm \Sigma}_k \bm U = \bm
U_1' \widehat{\bm \Sigma}_k \bm U_1 \oplus \bm 0_{p-q}$ $(k = 1, \ldots,
K)$. Thus, $\bm U' \tilde{\bm \Sigma}_k \bm U = \alpha_k \{(1 - \lambda) \bm U'
...
the decision rule in \eqref{eq:hdrda} is equivalent to
	\begin{align}
		D_{HDRDA}(\bm x) &= \argmin_k  (\bm x - \xbar_k)' \bm U_1 \bm W_k^{-1} \bm U_1' (\bm x - \xbar_k) + \log | \bm W_k |. \label{eq:hdrda-decomposed}
	\end{align}
\end{thm}
\begin{proof}
...
\mathbb{R}_{p \times 1}$, we have that
	\begin{align*}
	(\bm x - \xbar_k)' \tilde{\bm\Sigma}_k^{+}(\bm x - \xbar_k)  + \log |\tilde{\bm\Sigma}_k| &= (\bm x - \xbar_k)' \bm U_1 \bm W_k^{-1} \bm U_1' (\bm x - \xbar_k)\\
	&+ \gamma^{+} (\bm x - \xbar_k)' \bm U_2 \bm U_2' (\bm x - \xbar_k) + \log | \bm W_k | + (p - q) \log \gamma.
	\end{align*}
...
$\bm U_2' (\bm x - \xbar_k)$ is constant for $k = 1, \ldots, K$.
\end{proof}
...
that the inverse and determinants of $\bm W_k$ can be written so that they are
...
Let $\bm W_k$ be defined as above $(k = 1, \ldots, K)$. Then, $|\bm W_k| = |\bm
\Gamma_k| |\bm Q_k|$ and
	\begin{align}
...
\bm U'$ by the singular value decomposition, we have the eigendecomposition
$\widehat{\bm\Sigma} = \bm U \bm D \bm U'$, where $\bm M \in \mathbb{R}_{N
  \times N}$ is orthogonal, $\bm D^{1/2} \in \mathbb{R}_{N \times N}^{\ge}$ is a
diagonal matrix consisting of the singular values of $\bm X$, and $\bm U \in
\mathbb{R}_{p \times N}$ is orthogonal. Now, $\bm X \bm X' = \bm M \bm D \bm M'$
is the eigenvalue decomposition of $\bm X \bm X'$, where $\bm D$ is the matrix
of eigenvalues of $\bm X \bm X'$ and the columns of $\bm M$ are the
corresponding eigenvectors. Hence, we obtain $\bm M$ and $\bm D$ from an
eigendecomposition of $\bm X \bm X' \in \mathbb{R}_{N \times N}$. Next, we
compute $\bm U = \bm X' \bm M \bm D^{+/2}$, where $\bm D^{+/2} = \bm D_q^{-1/2}
\oplus \bm 0_{N-q}$. We then determine $q$ by computing the number of
numerically nonzero eigenvalues present in $\bm D$, where $q$ is the number of
