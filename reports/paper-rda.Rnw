...
\usepackage{graphicx, amsmath, amssymb, bm, natbib}
...
\markboth{J. A. Ramey, C. K. Stein \and D. M. Young}{High-Dimensional
...
\author{J. A. RAMEY}
\affil{Vaccine and Infectious Disease Division, Fred Hutchinson Cancer Research
  Center,\\ Seattle, Washington 98115, U.S.A. \email{johnramey@gmail.com}}

\author{C. K. STEIN \and D. M. YOUNG}
...
linear discriminant analysis proposed by \cite{Dudoit:2002ev} and the recently
...
\begin{align}
	D_{QDA}(x) = \argmin_{k}  (x - \xbar_k)^{T}\widehat{\Sigma}_k^{-1}(x - \xbar_k)  + \log |\widehat{\Sigma}_k|, \label{eq:qda}
\end{align}
where $\xbar_k$ and $\widehat{\Sigma}_k$ are the maximum likelihood
estimators for $\mu_k$ and $\Sigma_k$, respectively, and $A^{T}$ denotes the
matrix transpose of $A \in \mathbb{R}_{m \times p}$. If we assume
...
$I_m$ be the $m \times m$ identity matrix, and let $0_{m \times p}$ be the $m
\times p$ matrix of zeros, such that $0_m$ is understood to denote $0_{m \times
  m}$. For $c \in \mathbb{R}$, let $c^+ = 1/c$ if $c \ne 0$ and $0$
otherwise. Let $A \oplus B$ denote the direct sum of $A \in \mathbb{R}_{r \times
  r}$ and $B \in \mathbb{R}_{s \times s}$ \citep[Chapter
  1]{Lutkepohl:1996uz}. Let $A^+$ and $\mathcal{N}(A) = \{z \in \mathbb{R}_{p
  \times 1} : A z = 0_{m \times 1}\}$ denote the Moore-Penrose pseudoinverse and
null space of $A \in \mathbb{R}_{m \times p}$, respectively. Let $V^{\perp} =
\{y \in \mathbb{R}_{p \times 1} : y^{T}v = 0\ \forall v \in V \}$ denote the
orthogonal complement of a vector space $V \subset \mathbb{R}_{p \times 1}$.
...
analysis classifier
\begin{align}
	D_{RDA}(x) = \argmin_{k}  (x - \xbar_k)^{T}\widehat{\Sigma}_k(\lambda, \gamma)^{-1}(x - \xbar_k)  + \log |\widehat{\Sigma}_k(\lambda, \gamma)|. \label{eq:rda}
\end{align}
...
%	&= \left( \frac{1 - \lambda}{n_k} + \frac{\lambda}{N} \right)\sum_{i=1}^N I(y_i = k) x_i x_i^{T} +  \frac{\lambda}{N} \sum_{i=1}^N I(y_i \ne k) x_i x_i^{T} \nonumber \\
	&= \sum_{i=1}^N c_{ik}(\lambda) x_i x_i^{T},\label{eq:sig-lambda-alternative2}
\end{align}
...
each of the $N$ observations in estimating $\Sigma_k$. Hence, for $0 < \lambda
...
[\sqrt{c_{1k}(1)} x_1^{T}, \ldots, \sqrt{c_{Nk}(1)} x_N^{T}]^{T}$ such that
$\widehat{\Sigma} = X^{T} X$. Similarly, for $\lambda = 0$, we define
$X_k = [\sqrt{c_{1k}(0)} x_1^{T}, \ldots, \sqrt{c_{Nk}(0)} x_N^{T}]^{T}$ such
that $\widehat{\Sigma}_k = X_k^{T} X_k$. In Figure
...
\begin{align}
	D_{HDRDA}(x) = \argmin_{k}  (x - \xbar_k)^{T}\tilde{\Sigma}_k^{+}(x - \xbar_k)  + \log |\tilde{\Sigma}_k|. \label{eq:hdrda}
\end{align}
For $\gamma > 0$, $\tilde{\Sigma}_k$ is nonsingular such that
$\tilde{\Sigma}_k^{-1}$ can be substituted for $\tilde{\Sigma}_k^{+}$ in
\eqref{eq:hdrda}. If $\gamma = 0$, we explicitly set $|\tilde{\Sigma}_k|$ equal
to the product of the positive eigenvalues of $\tilde{\Sigma}_k$. Following
\cite{Friedman:1989tm}, we select $\lambda$ and $\gamma$ from a grid of
candidate models via cross-validation \citep{Hastie:2008dt}. We provide an
...
of $\widehat{\Sigma}$ in \eqref{eq:pooled-cov}. We show that the matrix
...
$\widehat{\Sigma}$, further reducing the computational costs of our proposed
classifier.
...
\ldots, K$. Hence, $0 = z^{T} \widehat{\Sigma}_k(\lambda) z = (1 -
\lambda) z^{T} \widehat{\Sigma}_k z + \lambda z^{T} \widehat{
...
\mathcal{N}(\widehat{\Sigma})$. Similarly, we have that $0 = z^{T}
\widehat{\Sigma} z = N^{-1} \sum_{k = 1}^K n_k z^{T} \widehat{
...
$\widehat{\Sigma}$. Let $\widehat{\Sigma} = U D U^{T}$ be the
...
Let $\widehat{\Sigma} = U D U^{T}$ be the eigendecomposition of
$\widehat{\Sigma}$ as above, and suppose that rank$(\widehat{\Sigma}) =
q \le p$. Then, we have
\begin{align}
	\tilde{\Sigma}_k &= U(W_k \oplus \gamma I_{p-q})U^{T} \quad
  (k = 1, \ldots, K),\label{eq:rda-matrix}
\intertext{where}
W_k &= \alpha_k \{(1 - \lambda) U_1^{T} \widehat{\Sigma}_k U_1 + \lambda D_q\} + \gamma I_{q}.\label{eq:Wk}
\end{align}
\end{lemma}
\begin{proof}
...
0_{p \times (p - q)}$. Hence, $U^{T} \widehat{\Sigma}_k U = 
U_1^{T} \widehat{\Sigma}_k U_1 \oplus 0_{p-q}$ $(k = 1, \ldots,
K)$. Thus, $U^{T} \tilde{\Sigma}_k U = \alpha_k \{(1 - \lambda) U^{T}
\widehat{\Sigma}_k U + \lambda D\} + \gamma I_p$, and
\eqref{eq:rda-matrix} holds because $U$ is orthogonal.
\end{proof}
...
Let $U_2$ be defined as above. Then, for all $x \in \mathbb{R}_{p \times
  1}$, $U_2^{T} (x - \xbar_k) = U_2^{T} (x - \xbar_{k'})$ $(1 \le k,
k' \le K)$.
\end{lemma}
\begin{proof}
Let $x \in \mathbb{R}_{p \times 1}$, and suppose that $1 \le k, k' \le K$.
Recall that $U_2 \in \mathcal{N}(\widehat{\Sigma})$, which implies that
$U_2^{T} \in \mathcal{C}(\widehat{\Sigma})^{\perp}$ \citep[Lemma
  1.2.5]{Kollo:2005vp}. Now, because $x_i \in \mathcal{C}(\widehat{
  \Sigma})$ $(i = 1, \ldots, N)$, $U_2^{T} x_i = 0_{(p-q) \times 1}$.
Hence, $0_{(p-q) \times 1} = \sum_{i=1}^N \beta_i U_2^{T}x_i = U_2^{T}(\xbar_k -
\xbar_{k'})$, where $\beta_i = (n_k n_{k'})^{-1} \{ I(y_i = k) n_{k'} - I(y_i =
k') n_k \}$. Therefore, $U_2^{T} (x - \xbar_k) = U_2^{T} (x - \xbar_{k'})$.
\end{proof}
...
the decision rule in \eqref{eq:hdrda} is equivalent to
	\begin{align}
		D_{HDRDA}(x) &= \argmin_k  (x - \xbar_k)^{T} U_1 W_k^{-1} U_1^{T} (x - \xbar_k) + \log | W_k |. \label{eq:hdrda-decomposed}
	\end{align}
\end{thm}
\begin{proof}
From \eqref{eq:rda-matrix}, we have that $\tilde{\Sigma}_k^{+} = U(
W_k^{-1} \oplus \gamma^{+} I_{p-q} )U^{T}$ and $|\tilde{\Sigma}_k| =
\gamma^{p-q}| W_k |$ $(k = 1, \ldots, K)$. Therefore, for all $x \in
\mathbb{R}_{p \times 1}$, we have that
	\begin{align*}
	(x - \xbar_k)^{T} \tilde{\Sigma}_k^{+}(x - \xbar_k)  + \log |\tilde{\Sigma}_k| &= (x - \xbar_k)^{T} U_1 W_k^{-1} U_1^{T} (x - \xbar_k)\\
	&+ \gamma^{+} (x - \xbar_k)^{T} U_2 U_2^{T} (x - \xbar_k) + \log | W_k | + (p - q) \log \gamma.
	\end{align*}
...
$U_2^{T} (x - \xbar_k)$ is constant for $k = 1, \ldots, K$.
\end{proof}
...
calculating the inverse and determinants of $W_k$ can be further improved via
...
\Gamma_k| |Q_k|$ and
	\begin{align}
		W_k^{-1} &= \Gamma_k^{-1} - \alpha_k(1 - \lambda) \Gamma_k^{-1} U_1^{T} X_k^{T} Q_k^{-1} X_k U_1 \Gamma_k^{-1},\label{eq:W_k_inv}
	\end{align}
where $Q_k = I_{n_k} + \alpha_k(1 - \lambda) X_k U_1 
\Gamma_k^{-1} U_1^{T} X_k^{T}$ and $\Gamma_k = \alpha_k \lambda D_q +
\gamma I_q$.
\end{proposition}
\begin{proof}
...
\alpha_k (1 - \lambda) U_1^{T} X_k^{T}$, $T = I_{n_k}$, and $C =
...
Conveniently, we see that the inverse and determinant of $W_k$ are each in terms
of the inverse and determinant of $Q_k \in \mathbb{R}_{n_k \times n_k}$,
respectively, as well as the diagonal matrix $\Gamma_k \in \mathbb{R}_{q \times
  q}$. This formulation is especially advantageous when the number of classes
$K$ is large because $n_k$ is then potentially much smaller than $q$. The
expressions given in Proposition \ref{proposition:hdrda-W_k} also expedite the
selection of $\lambda$ and $\gamma$ via cross-validation. The most
time-consuming matrix operations involved in computing $W_k^{-1}$ and $|W_k|$ is
$X_k U_1 \in \mathbb{R}_{n_k \times q}$, which is independent of $\lambda$ and
$\gamma$. Hence, we need calculate $X_k U_1$ only once for each cross-validation
fold. The subsequent operations in calculating $W_k^{-1}$ and $|W_K|$ can be
simply updated for different pairs of $\lambda$ and $\gamma$.

Thus far, we have constructed each $\tilde{\Sigma}_k$ in terms of the first $q$
eigenvectors of $\widehat{\Sigma}$. By utilizing a technique similar to the
so-called kernel trick, we can obtain $U_1$ by computing the eigendecomposition
of a much smaller $N \times N$ matrix when $p \gg N$
\citep{Hastie:2008dt}. Recall that the pooled sample covariance matrix is
$\widehat{\Sigma} = X^{T} X$. Then, because $X = M D^{1/2} U^{T}$ by the
singular value decomposition, we have the eigendecomposition $\widehat{\Sigma} =
U D U^{T}$, where $M \in \mathbb{R}_{N \times N}$ is orthogonal, $D^{1/2} \in
\mathbb{R}_{N \times N}^{\ge}$ is a diagonal matrix consisting of the singular
values of $X$, and $U \in \mathbb{R}_{p \times N}$ is orthogonal. Now, $X X^{T}
= M D M^{T}$ is the eigenvalue decomposition of $X X^{T}$, where $D$ is the
matrix of eigenvalues of $X X^{T}$ and the columns of $M$ are the corresponding
eigenvectors. Hence, we obtain $M$ and $D$ from an eigendecomposition of $X
X^{T} \in \mathbb{R}_{N \times N}$. Next, we compute $U = X^{T} M D^{+/2}$,
where $D^{+/2} = D_q^{-1/2} \oplus 0_{N-q}$. We then determine $q$, the number
of numerically nonzero eigenvalues present in $D$, by calculating the number of
eigenvalues that exceeds some tolerance value, say, $\epsilon = 1 \times
10^{-6}$. After calculating $q$, we extract $U_1$ by retaining only the first
$q$ columns of $U$.
...
option was available. For our classifier, we set $\alpha_k = 1$ $(k = 1, \ldots,
...
its misclassification estimates for small values of $d$ and yielded superior
...
{\tt sparsediscrim} R package was comparable to that of the diagonal classifiers
in terms of computational runtime. Moreover, when preliminary variable selection
was applied to the data in our classification study, the time to perform model
selection with our proposed classifier was nearly instantaneous.

\section*{Supplementary material}
\label{SM}
Supplementary material available at \Bka\ online includes the graphical summary
