...
small-sample, high-dimensional data. In Table \ref{tab:data-summary}, we
summarize the four microarray data sets. Our competitors include diagonal linear
...
\caption{Averages of the 1000 error-rate estimates of the competing classifiers
  ...
  standard errors are no greater than \Sexpr{max(error_rates[["SE_Error"]])}.}
