...
\citep{Clemmensen:2011kr}. The \emph{HDRDA} classifier allows us to relax the
...
general shrinkage term to the \emph{RDA} classifier to establish our \emph{HDRDA}
classifier.
...
our \emph{HDRDA} classifier is competitive with and often superior to several
recent classifiers designed for small-sample, high-dimensional data. In
particular, we compare our \emph{HDRDA} classifier with the \emph{DLDA}
...
\ref{fig:hdrda-contours} we demonstrate the effect of $\lambda$ on the contours
of five multivariate normal populations with unequal covariance matrices.
...
\eqref{eq:hdrda}. Lastly, we exploit the so-called fast singular value
...
We now present our main result, where we decompose the \emph{HDRDA} decision rule
...
Lemma \ref{lemma:RDA-constant-term}. Our main result reduces \eqref{eq:hdrda} to
an equivalent, more computationally efficient and more stable decision rule.
...
the \emph{HDRDA} decision rule in \eqref{eq:hdrda} is equivalent to
	\begin{align}
		D_{HDRDA}(\bm x) &= \argmin_k  (\bm x - \xbar_k)' \bm U_1 \bm W_k^{-1} \bm U_1' (\bm x - \xbar_k) + \ln | \bm W_k |. \label{eq:rda-decomposed}
	\end{align}
...
Thus far we have shown that the \emph{HDRDA} classifier in
...
\emph{HDRDA} classifier. First, we demonstrate that the inverse and determinants
...
improvements to the computational efficiency of the \emph{HDRDA} classifier.
...
our proposed \emph{HDRDA} classifier often yields superior classification
accuracy to the diagonal classifiers considered, confirming the assertion of
\cite{Mai:2012bf}. Furthermore, the \emph{HDRDA} classifier is able to compete
