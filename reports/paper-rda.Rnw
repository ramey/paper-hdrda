...
results$GRDA <- as.numeric(results$GRDA)
...
grda_optimal <- subset(results, select = c(data_set, d, lambda, gamma))
grda_optimal$lambda <- factor(grda_optimal$lambda, levels = seq_lambda)
grda_optimal$gamma <- factor(grda_optimal$gamma, levels = seq_gamma)

grda_optimal <- ddply(grda_optimal, .(data_set, d), function(x) {
...
matrix operations and involves a time-consuming model-selection procedure. We
propose a novel modification of the \emph{RDA} (\emph{GRDA}) classifier that
provides a straightforward interpration and employs a novel dimension-reduction
technique to enable a computationally efficient model-selection procedure, thus
overcoming the computational shortcomings of the \emph{RDA}
classifier. Moreover, we demonstrate with several high-dimensional microarray
data sets that our proposed classifier is competitive and often superior to
several recently proposed sparse and regularized classifiers in terms of
classification accuracy. Finally, we provide an implementation of the
\emph{GRDA} classifier in the {\tt sparsediscrim} R package, available on CRAN.
...
$\widehat{\bm\Sigma}$. In Figure \ref{fig:grda-contours} we demonstrate the
effect of $\lambda$ on the contours of five multivariate normal populations with
unequal covariance matrices. For brevity, when $\lambda = 1$, we define $\bm X =
...
\[ \left[\text{Figure \ref{fig:grda-contours} goes about here }\right]\]
...
$d$. Furthermore, Figure \ref{fig:error-rates} emphasizes two strengths of the
...
\emph{GRDA} classifier. For brevity, we include only a graphical summary of the
...
found in the Supplementary Information. In Figure \ref{fig:optimal-shipp}, the
distribution of the optimal values of $\lambda$ selected through
cross-validation has a notable interesting bathtub shape, resembling a Beta
distribution with shape parameters $1/2$ and $1/2$. Specifically, the optimal
values of $\lambda$ are typically near 0 or near 1, corresponding to the
\emph{QDA} and \emph{LDA} classifiers, respectively. We noticed a similar
distribution of optimal values of $\lambda$ for the other three data sets.
...
of variables resulted in the \emph{GRDA} classifier preferring the more
parsimonious assumption of equal covariance matrices. In this instance we can
see the flexibility of the \emph{GRDA} classifier to employ the more realistic
...
classifier have been proposed by employing for improved classification
performance. However, the \emph{LDA} classifier includes the strong assumption
that the covariance matrices of each class are equal. Ideally, we could model
the class covariance matrices as unrestricted and unequal in the \emph{QDA}
classifier, but it is well known to have a large number of parameters that must
be estimated and require a large number of samples, which are often unavailable
in the high-dimensional setting. With this mind, \cite{Friedman:1989tm} proposed
the \emph{RDA} classifier as a compromise between the \emph{LDA} and \emph{QDA}
...
With this in mind, we have adapted the \emph{RDA} classifier for usage with
modern small-sample, high-dimensional data sets. Our model formulation
facilitates a rapid classification of high-dimensional data by employing a novel
dimension reduction technique. Moreover, our 10-fold cross-validation procedure
typically requires a small runtime for data sets having large $p$. When
...
operations employed in the \emph{GRDA} classifier. Additionally, we have
provided an implementation of the \emph{GRDA} classifier in the {\tt
...
\emph{GRDA} classifier to include a large family of shrinkage methods, such as
...
\cite{Xu:2009fl}. Our formulation of the \emph{GRDA} classifier in terms of the
...
In our simulation study, we compared the \emph{GRDA} classifier to several
...
\cite{Mai:2012bf} have noted that diagonal classifiers can often yield inferior
classification performance to other classification methods. We have shown that
our proposed \emph{GRDA} classifier often yields superior classification
accuracy to the diagonal classifiers considered, confirming the assertion of
\cite{Mai:2012bf}. Furthermore, the \emph{GRDA} classifier is able to compete
with the diagonal classifiers in terms of speed by incorporating a novel
dimension-reduction technique. Although the diagonal classifiers are clearly
faster to compute, the differences in runtime for small-sample, high-dimensional
data sets is hardly noticeable.
...
\begin{figure}
<<grda_contours, fig.width=8, fig.height=8, out.width='.49\\linewidth', fig.show='hold'>>=
...
\begin{figure}
<<optimal_estimates>>=
grda_optimal$lambda <- factor(grda_optimal$lambda)
grda_optimal$gamma <- factor(grda_optimal$gamma)
p <- ggplot(subset(grda_optimal, data_set == "shipp" & d %in% c(100, 500, 1000)), aes(lambda))
p <- p + geom_bar(aes(fill = gamma, weight = value))
p <- p + facet_grid(d ~ .) + theme_bw()
p
@ 
\caption{Counts of the optimal tuning-parameter estimates for the \emph{GRDA}
